...
```sh
xcrun notarytool submit release/Snake-1.1.1-macos-arm64.dmg --keychain-profile <你的公证配置名> --wait
xcrun stapler staple release/Snake-1.1.1-macos-arm64.dmg
xcrun stapler validate release/Snake-1.1.1-macos-arm64.dmg
```
...
- `hdiutil verify` 和 DMG SHA-256 校验通过。
- 只读挂载 DMG，确认 Applications 链接指向 `/Applications`；将应用复制到独立临时目录后，`codesign --verify --deep --strict` 通过。
- 独立副本可定位应用内部资源包；7 个 Metal shader 函数实际编译通过，没有启动应用。
- 随包语言资源完整：`zh-Hans` 为源语言，`en` 共 536 条；两种语言的 `InfoPlist.strings` 齐备。
- 动态链接检查仅包含随包 Rust dylib 和 Apple 系统框架／库，无 Homebrew 或工作目录动态库路径。
- `swift test --disable-sandbox --filter 'WorkspaceDraggingTests|SFTPShortcutTests'`：24 项通过；`--filter LocalizationTests`：10 项通过。
- 同名产物保护已验证，重复执行打包不会覆盖已有 DMG。
- `spctl --assess` 拒绝当前 ad-hoc 包，符合无 Developer ID / 公证的限制；签名完整性通过不代表 Gatekeeper 放行。
- 未执行干净 Mac 安装、GUI 或完整挂载回归；页面由用户验收。构建仍有 SwiftTerm 既有未使用返回值警告。
