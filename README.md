...
- 打开远程文件的缓存目录可在设置中更换（默认系统缓存目录），并按保留时长与容量上限自动清理，默认 7 天 / 500 MB；也可随时手动清理。回收只针对 Snake 自己下载的副本，不会改动所选目录中的其他文件。
...
- 上传与下载支持配置化并行传输，默认超过 50 MB 使用 4 个连接分片。上传支持分片续传与隐藏暂存文件安全覆盖；SFTP-only 兼容路径失败重试重新上传。SFTP 分栏/窗口之间保留远程文件拖拽互传。同时进行多个传输时，同一台主机的分片与逐文件传输连接受「每主机连接数」限制（默认 8，可设 2–16）；每个标签的常驻连接、每个上传批次的 1 条操作连接不计入。超出的传输会排队并在记录里显示“等待中”。
