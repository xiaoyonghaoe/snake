...
- 上传与下载支持配置化并行传输，默认超过 50 MB 使用 4 个连接分片。上传支持分片续传与隐藏暂存文件安全覆盖；SFTP-only 兼容路径失败重试重新上传。SFTP 分栏/窗口之间保留远程文件拖拽互传。同时进行多个传输时，对同一台主机的传输连接总数受「每主机连接数」限制（默认 8，可设 2–16），超出的传输会排队并在记录里显示“等待中”。
